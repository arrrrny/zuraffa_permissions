...
## Roles & access control (built on Zuraffa)

Beyond the runtime port, the package models an RBAC layer entirely on
Zuraffa patterns: `Permission`, `Role`, `UserPermission`, `UserRole` and
`RolePermission` are Zorphy entities (FR-001); persistence flows through
Zuraffa datasources/repositories with a pure-Dart
`InMemoryPermissionStore` default (FR-002); and every business rule —
`CheckPermissionUseCase`, `ListUserPermissionsUseCase`,
`AssignRoleToUserUseCase`, `RemoveRoleFromUserUseCase`,
`GrantPermissionToUserUseCase`, `RevokePermissionFromUserUseCase`,
`GrantPermissionToRoleUseCase` — is a Zuraffa `UseCase` resolvable from
GetIt via `registerPermissionDependencies` (FR-003/FR-004).

```dart
registerPermissionDependencies(getIt);
final check = getIt<CheckPermissionUseCase>();
final allowed = await check.execute(
  const CheckPermissionParams(userId: 'u1', permissionId: 'perm.camera.read'),
  null,
);
```
...
```text
packages/
├── zuraffa_permissions/                      # app-facing package (this one)
